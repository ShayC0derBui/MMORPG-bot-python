...
class AlbionBot:
    ...
    def __init__(self, window_offset, window_size, monitor, Pather):
        # create a thread lock object
        self.visited_angles = []
        self.backtracking_point = None
        self.lock = Lock()
...
#
    ...
    def run(self):
        while not self.stopped:
            if self.state == BotState.INITIALIZING:
                ...
                if not self.have_stopped_moving():
                    ...
                    if self.state == BotState.MOVING:
                        ...
                        where = self.pather.where()
                        # self.click_history.append(where)
                        vector = self.pather.create_vector(self.backtracking_point, where)
                        angle = self.pather.angle_with_x_axis(vector)
                        self.visited_angles.append(angle)
                        self.lock.release()
            ...
            elif self.state == BotState.PATHING:
                print("pathing")
                try:
                    self.backtracking_point = self.pather.pathing()
                except Exception as e:
                    print("An error occurred:", e)
                print('stationed')
                self.lock.acquire()
                self.state = BotState.STATIONED
                self.lock.release()
            elif self.state == BotState.STATIONED:
                self.lock.acquire()
                self.state = BotState.SEARCHING
                self.lock.release()
